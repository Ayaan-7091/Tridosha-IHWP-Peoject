  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="bg-white rounded-lg shadow-lg p-8 mb-8">
        <h2 className="text-3xl font-bold text-green-900 mb-6">Prakriti Analysis Questionnaire</h2>

        {!result ? (
          <div className="space-y-6">
            {questions.map((q, idx) => (
              <div key={q.id} className="pb-6 border-b border-green-100">
                <p className="font-semibold text-gray-800 mb-4">{idx + 1}. {q.text}</p>
                <div className="space-y-2">
                  {Object.entries(q.options).map(([dosha, label]) => (
                    <label key={dosha} className="flex items-center p-3 bg-green-50 hover:bg-green-100 rounded cursor-pointer">
                      <input
                        type="radio"
                        name={`q${q.id}`}
                        value={dosha}
            ...
            <div className="flex gap-4 mt-8">
              <button
                ...
                className="flex-1 bg-blue-600 text-white py-3 rounded-lg hover:bg-green-700 disabled:bg-gray-300 font-semibold">
                Calculate Prakriti
              </button>
              <button
                onClick={onBack}
                className="px-6 bg-red-600 text-white py-3 rounded-lg hover:bg-gray-500 font-semibold">
                Back
              </button>
            </div>
          </div>
          ...
          <div className="text-center space-y-8">
            ...
            <div>
              <h3 className="text-2xl font-bold text-green-900 mb-2">Your Prakriti</h3>
              <p className="text-5xl font-bold text-green-600 mb-6">{result}</p>
