  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="bg-white rounded-lg shadow-lg p-8">
        <h2 className="text-3xl font-bold text-green-900 mb-8">Daily Routine for {prakriti}</h2>

        <div className="space-y-4">
          {Object.entries(schedule).map(([key, time]) => (
            <div key={key} className="flex items-center p-4 bg-green-50 rounded-lg">
              <div className="w-24 font-bold text-green-900 capitalize">{key.replace(/([A-Z])/g, ' $1').trim()}:</div>
